...
def add_job_change(read_text, full_name, id, birthdate, job_title, position_from):
...
index_file = int(input('Enter: '))

if index_file == 1:
    with open(path + '\job_change.txt') as text:
        read_text = text.read()
        result = add_job_change(read_text, 'Ivo Marek', '123458', '27.4.1965', 'mechanik', '27.1.1965')
        print(result)